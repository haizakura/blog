<template>
  <div class="copyright-block vp-doc relative bg-bg-soft rounded-lg pa-4 overflow-hidden" :key="page.relativePath">
    <div class="relative z-1">
      ...
      <div class="mt-4 flex gap-16">
        <div>
          ...
          <div class="font-bold">{{ theme.author }}</div>
        </div>
        <div>
          <div class="text-sm">发布于</div>
          <div class="font-bold">{{ createDate }}</div>
        </div>
        <div>
          <div class="text-sm">许可协议</div>
          <a class="font-bold" :href="theme.licenseLink" target="_blank">{{ theme.license }}</a>
        </div>
      </div>
    </div>
    ...
    <svg
      v-if="licenseIsCC"
      class="absolute top--25% right--12 h-150% opacity-10"
      fill="currentColor"
